      ...
      {pData.map((p, i) => (
        <Card key={i}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            ...
            <div style={{ textAlign: 'right' }}><input type="number" value={paycheckOverrides[i] !== undefined ? paycheckOverrides[i] : paySettings.amount} onChange={e => setPaycheckOverrides(prev => Object.assign({}, prev, {[i]: e.target.value}))} style={{ fontWeight: 700, fontSize: 18, color: C.green, background: 'transparent', border: 'none', textAlign: 'right', width: 130, padding: 0 }} /><div style={{ fontSize: 10, color: C.charcoalLight }}>paycheck</div></div>{p.bills.length === 0 ? <div style={{ color: C.charcoalLight, fontSize: 12, padding: '10px 0', borderTop: `1px solid ${C.creamDark}` }}>No bills due this period 🎉</div> : (
